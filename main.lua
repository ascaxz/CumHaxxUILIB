...
local MainWindow = Library:CreateWindow("Cumhaxx - Private", 500, 620, 32)
local Main = MainWindow:CreateCategory("Main")
local Visual = MainWindow:CreateCategory("Visual")
local Movement = MainWindow:CreateCategory("Movement")
local Misc = MainWindow:CreateCategory("Misc")
local Settings = MainWindow:CreateCategory("Settings")

local AimbotSection = Main:CreateSection("Aimbot")
local AimbotToggle = Main:CreateToggle("Aimbot", false, function()
end)
local SilentAimToggle = Main:CreateToggle("Silent Aim", false, function()
end)
local FOV = Main:CreateSlider("FOV", -10, 1000, 1000, function()
end)
local Combo = Main:CreateComboBox("Hitpart", function()
end, {
	"Head",
	"Torso",
	"Random"
}, "Head")
local Button = Main:CreateButton("Please kill myself", function()
	if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Head") then
		game.Players.LocalPlayer.Character.Head:Destroy()
	end
	print("Died")
end)
local TextBox = Main:CreateTextBox("Textbox lol", function(text)
	print(text)
end)

local ESPSection = Visual:CreateSection("ESP")
local Esp = Visual:CreateToggle("Enabled", false, function()
end)
local SexManIsBehindMe = Visual:CreateButton("Sex me", function()
	print("A")
end)

local color_picker = Settings:CreateColorPicker({
	ESP = game.Lighting.Ambient
}, function(C)
